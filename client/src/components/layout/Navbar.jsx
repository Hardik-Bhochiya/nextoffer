        ...
        <div ref={dropdownRef} className="relative">
          ...
          {isDropdownOpen && (
            <div className="absolute right-0 top-full mt-2 w-72 bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl p-3 z-50 animate-fadeIn space-y-3">
              ...
              {/* Option 2: Role-Specific Recommendations Feature Box */}
              <div className="p-3 rounded-2xl bg-slate-950/90 border border-slate-800 space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-indigo-400 flex items-center gap-1">
                    <Sparkles className="w-3 h-3" /> Role Recommendations
                  </span>
                  <span className="text-[9px] text-slate-500 font-medium">Curated Tracks</span>
                </div>

                <div className="space-y-1">
                  {metrics?.recommendedRoadmapTitles?.slice(0, 3).map((title, i) => (
                    <div key={i} className="flex items-center gap-1.5 text-[11px] text-slate-300 truncate">
                      <CheckCircle2 className="w-3 h-3 text-emerald-400 shrink-0" />
                      <span className="truncate">{title}</span>
                    </div>
                  )) || (
                    <p className="text-[11px] text-slate-500 italic">Select a target role to view recommendations</p>
                  )}
                </div>
                ...
                <button
                  ...
                  className="w-full mt-1.5 py-1.5 rounded-lg bg-indigo-600/80 hover:bg-indigo-600 text-white text-[11px] font-semibold text-center transition-colors block shadow-sm"
                >
                  View Full Recommendations & Boost
                </button>
              </div>
