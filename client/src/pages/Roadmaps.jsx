...
export const Roadmaps = () => {
  ...
  const filteredRoadmaps = selectedGroup === 'All'
    ? roadmaps
    : roadmaps.filter(r => (r.categoryGroup || '').toLowerCase() === selectedGroup.toLowerCase() || r.category?.toLowerCase().includes(selectedGroup.toLowerCase()));
  ...
  return (
    <div className="space-y-8 animate-fadeIn max-w-6xl mx-auto pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-100 flex items-center gap-2">
            <GitBranch className="w-6 h-6 text-indigo-400" /> Software Engineering Placement Roadmaps
          </h1>
          <p className="text-xs text-slate-400 mt-1">
            Curated 2–3 roadmaps per core domain tailored for Frontend, Backend, Full-Stack, DSA, System Design, Core CS & DevOps.
          </p>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 px-4 py-2.5 rounded-2xl flex items-center gap-3 self-start sm:self-auto shadow-sm">
          <div className="text-right">
            <p className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider">Active Tracks: {enrolledCount}</p>
      ...
      <div className="flex flex-wrap gap-2 pb-1 border-b border-slate-800/80">
        {groups.map((group) => {
          const groupRoadmaps = group === 'All' ? roadmaps : roadmaps.filter(r => (r.categoryGroup || '').toLowerCase() === group.toLowerCase() || r.category?.toLowerCase().includes(group.toLowerCase()));
          const hasEnrolled = groupRoadmaps.some(r => r.isEnrolled);
          ...
          return (
            ...
            >
              <span>{group}</span>
              <span className="text-[10px] text-slate-400 font-mono">({groupRoadmaps.length})</span>
              {hasEnrolled && (
            ...
            >
              ...
              <div className="flex items-center justify-between pt-1">
                <span className="text-[11px] text-slate-400">
                  {isEnrolled ? `✓ ${completed}/${total} Milestones Done` : 'Not enrolled yet'}
                </span>
                <button
                  type="button"
                ...
                >
                  {isEnrolled ? (
                    <>
                      <Unlock className="w-3.5 h-3.5" /> Enrolled (Leave Track)
                    </>
                  ) : (
                    <>
                      <Lock className="w-3.5 h-3.5" /> Enroll in Track
                    </>
                  )}
                </button>
              </div>
              ...
              <div className="space-y-2 max-h-80 overflow-y-auto pr-1">
                {!isEnrolled && (
                  <div className="p-4 rounded-2xl bg-slate-950/60 border border-slate-800/80 text-center space-y-1 my-1">
                    ...
                    <p className="text-[11px] text-slate-500">Click "Enroll in Track" above to unlock milestones and track progress.</p>
                  </div>
                )}
